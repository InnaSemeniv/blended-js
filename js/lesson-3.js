...
const getResult = (array) => {
    return array.flatMap(item => item.values);
};
...
const filterUser = user.filter(user => user > 20); 
...
class Calculator {
    ...
    divide(value) {
        if (value === 0) {
            throw new Error("Помилка: неможливо ділити на 0");
        }
        this.value /= value;
        return value;
    }
...
const client = new Client('inna123', 'inna@gmail.com');
